...
- **Review evidence.** Raw rows via `to_string()` for every number. Runtime per step recorded; the threshold is `cfg.run_step_timeout_minutes` (20) per step, or `cfg.run_long_step_timeout_minutes` (60) for the steps in `cfg.run_long_steps` (3.4, 3.5, 3.6, 6.1, 6.2, 6.3, 6.6). Over the threshold: stop and report (rule 10).
...
**Builds.** `regime/models/gmm.py`: `run_expanding_gmm(z: pd.DataFrame, K: int, cfg: Config, chain_to: np.ndarray) -> pd.DataFrame` — same refit dates as 3.4; at each D, `cfg.gmm_n_restarts` fits of `GaussianMixture(n_components=K, covariance_type=cfg.gmm_covariance_type, n_init=cfg.gmm_n_init, max_iter=cfg.gmm_max_iter, tol=cfg.gmm_tol, reg_covar=cfg.gmm_reg_covar, random_state=cfg.run_seed + i)` on model-input rows `[features_from, D]`, kept by highest `lower_bound_`, restarts table (`restart, seed, lower_bound, n_iter, converged`) to `outputs/tables/gmm_restarts_<D>.csv`; chaining by `chain_permutation` applied to `means_`, `covariances_`, `weights_` and `precisions_cholesky_` — the first GMM refit chains to `chain_to` = the HMM's first anchored refit means, every later one to the previous GMM refit (convention 16); "filtered" probability at t (D ≤ t < next D) = `predict_proba(z_t)` under the anchored parameters in force. Frame: index `date`, columns `p0 … p{K−1}, refit_date`, written to `cfg.outputs_gmm_filtered_probs`.
...
**Amended (session 4, reviewer answer to Q1).** Step 4.2 also writes `outputs/tables/conditional_stats_refit_split.csv`, for `hmm_filtered` only. Every out-of-sample date carries the flag `run_started_on_refit`: true when the filtered label run containing that date began on a refit date (a run is a maximal block of consecutive equal filtered labels; the first date of the out-of-sample window starts a run). The table holds conditional statistics by `factor`, `state` and the flag, with `n`: columns `factor, state, run_started_on_refit, n, ann_mean, ann_std, sharpe`. This is how the 45% figure of `decisions/section_3_review.md` Q1 has its cost measured rather than engineered away.

**Amended (session 5 addendum, reviewer answer to section 4 Q2).** The refit split is bootstrapped. `conditional_stats_refit_split.csv` gains `sharpe_p05, sharpe_p95, excludes_zero` per (`factor`, `state`, `run_started_on_refit`), and `outputs/tables/conditional_refit_split_differences.csv` holds, per (`factor`, `state`), the difference between the two flags with its own interval: columns `factor, state, sharpe_diff, diff_p05, diff_p95, excludes_zero, n_false, n_true`, `sharpe_diff` signed refit minus not-refit. `bootstrap_refit_split(labels, factors, refits, cfg) -> (stats, differences)` uses the same `StationaryBootstrap` settings and the same seed as step 4.3, and **both flags are resampled in one draw** — the flag is a column of the resampled array beside the label and the returns — so each replication's difference is a like-for-like comparison on one resampled history. `conditional_stats_refit_split.csv` does not take `excess_sharpe`; only the `conditional_stats_<source>` tables do.

**Tests.** `tests/test_conditional.py::test_conditional_stats_by_hand` — a synthetic joined frame with two states and two factors; literal expected `n, ann_mean, ann_std, sharpe`. `tests/test_conditional.py::test_unassigned_rows_are_excluded_and_counted`. `tests/test_conditional.py::test_run_started_on_refit_flag` — a hand-built eight-row label series with two refit dates; the flag is asserted row by row against the literal expected booleans. `tests/test_conditional.py::test_refit_split_difference_zero_when_flags_identical` — an alternating label series in which each (state, flag) half holds identical returns, with `bootstrap_block_size` far above the sample length so every draw is a rotation: `sharpe_diff` and both percentiles are exactly 0.

**Review evidence.** The four tables in full via `to_string()`; unassigned counts per source with the dates; `conditional_stats_refit_split.csv` and `conditional_refit_split_differences.csv` in full.
...
**Amended (session 4).** Step 4.3 also writes, per source, `outputs/tables/conditional_differences_<source>.csv` with columns `factor, state_a, state_b, sharpe_diff, diff_p05, diff_p95, excludes_zero, n_a, n_b`: for every factor and every pair of states a < b, `sharpe_diff = sharpe_b − sharpe_a` on the observed sample, with the percentiles taken from the *same* bootstrap replications as the conditional Sharpes — the difference is formed inside each replication, never from two independently resampled marginals. `n_a` and `n_b` are the observed counts. `rules` has 4 states and so 6 pairs per factor; the HMM and GMM sources have `primary_K` states and 3 pairs per factor at K = 3.

It also writes `outputs/tables/unconditional_stats.csv` with columns `factor, n, ann_mean, ann_std, sharpe, sharpe_p05, sharpe_p95` over the same out-of-sample dates, **all months included** (no state conditioning, no `assigned` filter), so every conditional Sharpe has the pooled number it must be read against.

**Amended (session 5 addendum, reviewer answer to section 4 Q3).** Every `conditional_stats_<source>.csv` carries `excess_sharpe = sharpe - the factor's unconditional Sharpe` from `unconditional_stats.csv`, inserted immediately after `sharpe` by `add_excess_sharpe(stats, pooled)`; `unconditional_stats.csv` is therefore written before any conditional table. Mkt-RF pays 0.668 unconditionally over this window, so a conditional 0.712 is a level rather than a finding and the two must be readable side by side. Every `conditional_differences_<source>.csv` **keeps its columns unchanged**: a difference of two conditional Sharpes already nets the unconditional level out, so subtracting it again would subtract zero. `block_bootstrap_ci` keeps its nine-column return type; the column is added at write time in `run.py`.

**Tests.** `tests/test_conditional.py::test_pairwise_difference_zero_when_states_identical` — two states whose return rows are identical give `sharpe_diff` 0 and both percentiles 0. `tests/test_conditional.py::test_bootstrap_reproducible_with_config_seed` — two calls give identical tables. `tests/test_conditional.py::test_excludes_zero_rule` — a hand table of (p05, p95) pairs maps to the expected booleans. `tests/test_conditional.py::test_bootstrap_schema_and_n_unchanged` — `n` equals `conditional_stats`'s `n`; nine columns. `tests/test_conditional.py::test_excess_sharpe_is_conditional_minus_unconditional` — the column equals the cell's Sharpe less the pooled Sharpe recomputed in the test from the same returns, the nine original columns are unchanged and in place, and a pooled table missing a factor raises.

**Review evidence.** The four nine-column tables in full; `unconditional_stats.csv` in full, with `excess_sharpe` beside `sharpe`; the `conditional_differences_<source>` tables for `hmm_filtered` and `hmm_smoothed` in full; the count of pairwise differences whose interval excludes zero per source (question 2 input); wall-clock.
...
### Step 6.6 — rerun with the non-primary feature set

**Builds.** `run_robustness_nonprimary(cfg: Config) -> None` in `regime/run.py`: the main run's protocol with the model-input columns of the **non-primary** feature set — `cfg.features_core_no_level` (d = 7) if `features.primary` is `"core"`, and `cfg.features_core` (d = 8) if it is `"core_no_level"` — at `primary_K`, `cfg.hmm_covariance_type` ("full"), the main run's refit dates and first window, `run_expanding_hmm` and `run_smoothed_hmm`. State numbering is chained exactly as the main run's (convention 16). Section 3 already writes this run's classifier outputs under `robustness/<name>/`; step 6.6 adds the conditional statistics, the bootstrap and the timing grid on top of them. Conditional statistics and the block bootstrap are computed for `hmm_filtered` and `hmm_smoothed`, and the timing grid is restricted to those two sources. Outputs under `robustness/nolevel/` and `outputs/tables/timing_results_nolevel.csv`.
