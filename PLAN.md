...
### Step 6.6 — d = 7 rerun without `dgs10_level`

**Builds.** `run_robustness_nolevel(cfg: Config) -> None` in `regime/run.py`: the main run's protocol with model-input columns `cfg.features_core_no_level` (the seven core features without `dgs10_level`, d = 7), `primary_K`, `cfg.hmm_covariance_type` ("full"), the main run's refit dates and first window, `run_expanding_hmm` and `run_smoothed_hmm`. Anchoring is unchanged: the anchor feature (`dgs10_chg12`) and the tiebreak feature (`cpi_3m_ann`) are both still present in the reduced set. Conditional statistics and the block bootstrap are computed for `hmm_filtered` and `hmm_smoothed`, and the timing grid is restricted to those two sources. Outputs under `robustness/nolevel/` and `outputs/tables/timing_results_nolevel.csv`.

**Tests.** `tests/test_robustness.py::test_nolevel_model_input_has_seven_columns_without_dgs10_level`.

**Review evidence.** The headline row of `timing_results_nolevel.csv` beside the main run's headline row; the count of conditional cells whose bootstrap interval excludes zero, for both runs.
