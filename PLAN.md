...
- **Raw data.** Never overwritten. Every pull writes `data/raw/<source>/<series>_<pull_id>.parquet` with a `pulled_at` column and appends one row to `data/raw/manifest.csv`. Pinned snapshots are the `pull_id` keys in `config.toml`; the run always reads the pinned snapshot.
- **Tests.** `pytest -q` from the repo root passes at the end of every step. Tests that need synthetic data build it in the test. Tests that need pulled data read the pinned `pull_id` from `config.toml` and fail (not skip) if the file is absent. No test touches the network.
- **Review evidence.** Raw rows via `to_string()` for every number. Runtime per step recorded.
...
**Builds.** `splice_dollar_chg12(panel: pd.DataFrame, cfg: Config) -> pd.Series`: `m = ln(dtwexm / dtwexm.shift(L)) × 100`, `b = ln(dtwexbgs / dtwexbgs.shift(L)) × 100`; result equals `m` for dates < `cfg.features_dollar_splice_date` and `b` for dates ≥ it. Changes are spliced, never levels. If `b` at the splice date is NaN because DTWEXBGS begins in 2006-01 and has no observation 12 rows before 2006-12-31, that row is NaN, reported in the review file, and the session appends an `OPEN.md` item with the two options: keep 2006-12-31 and accept the dropped row, or move `dollar_splice_date` to 2007-01-31. The splice is not moved in this session.
...
**Review evidence.** The 12 rows either side of the splice date showing `dtwexm, dtwexbgs`, the two change series and `dollar_chg12` via `to_string()`; the value of `dollar_chg12` at 2006-12-31.
...
**Builds.** `run_robustness_10feat(cfg)`: `dataclasses.replace(cfg, sample_features_from=cfg.sample_robustness_from)` with model-input columns `core + robustness`, `primary_K`, the same refit dates, outputs under `robustness/10feat/` and `outputs/tables/timing_results_10feat.csv`. The z matrix is not recomputed: the standardised robustness columns already exist in `features_z.parquet` (step 2.3) and the run reads them. The first window for this run is `[2004-01-31, first_window_end]`, twelve rows for d = 10, which is fewer rows than parameters; the session runs it as specified, reports the fit (`converged`, degenerate states, singular-covariance errors if hmmlearn raises), and if the fit is unusable appends an `OPEN.md` item with two options (A: keep `first_window_end` and report the run as degenerate; B: a later first refit date for this variant only, e.g. 2009-12-31) and completes the rest of the section.

**Tests.** `tests/test_robustness.py::test_10feat_model_input_has_ten_columns_from_2004_01_31`.

**Review evidence.** The first-window restarts table; the headline row if the run completed; the `OPEN.md` item otherwise.
...
**Builds.** Delete everything under `data/processed/` except `.gitkeep` (raw data stays), run `python -m regime.run` (no `--pull`), which runs sections 1 to 7 from the pinned snapshots, appends one row per section to `outputs/tables/runtime.csv` (columns `run_started, section, seconds`), and then run it a second time and confirm every file under `outputs/` is byte-identical (idempotent) — sha256 of each file before and after, written to the review file.
...
**Review evidence.** `runtime.csv` in full; the sha256 table (path, sha_run1, sha_run2, identical); total wall-clock; any section over 20 minutes named.
