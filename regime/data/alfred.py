...
section 3). A month with no release on or before t is NaN; a decision date
before the first release of anything has all values NaN. There is no
fallback to the current vintage.
"""
...
def asof_from_releases(releases: pd.DataFrame, decision_dates: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    the value of the release with the greatest ``realtime_start <= t``, NaN if
    no such release exists for m.
    """
    ...
    # so that row r holds the latest value released on or before release date r.
